# Extraction report — batch 01

Generated 2026-09-17 09:24. Corpus `JUNE/`. 10 file(s), 12 page(s), 14s total.
...
| **Unhealthy pages** | 0 | **0/12** |
| Cells extracted | — | 2295 |
| Deterministic (`exact`) | — | 1760 |
| Pixel-derived | — | 514 |
| Needing human review | as low as possible | 454 (88.3% of pixel cells) |
| Structural flags (deterministic cells) | as low as possible | 21 |
| Column roles assigned | report | 55/116 (47.4%) |
...
| Overflow rows | a row put cells beyond the last column its header names | 6 |
| Sparse rows | a row's populated-cell count differs from its own section's most common width — usually a blank where the value is zero, which is expected, not a fault | 62 |
| Mixed-type columns | a column is numeric on some rows and prose on others, so something landed in the wrong band | 10 |
| Unheaded columns | data sits under a column the header never named | 3 |

Where a full stock quadruple was mapped, the row equation closes on **128 of 138** rows (92.8%). That is the strongest alignment evidence available: if a value had landed in the wrong column, the arithmetic would not balance.
...
- `003063_.pdf` p1 — 0 overflow row(s), 3 mixed-type column(s), 18 sparse row(s)
- `002621_160-61 (2).pdf` p1 — 0 overflow row(s), 2 mixed-type column(s), 17 sparse row(s)
- `IMG-20260630-WA0015 .jpg` p1 — 0 overflow row(s), 2 mixed-type column(s), 4 sparse row(s)
- `300176_.pdf` p1 — 0 overflow row(s), 1 mixed-type column(s), 5 sparse row(s)
